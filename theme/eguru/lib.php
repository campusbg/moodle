...
function theme_eguru_get_main_scss_content($theme) {
    ...
    } else if ($filename == 'color_scheme-5.scss') {
        $scss .= file_get_contents($CFG->dirroot.'/theme/eguru/scss/preset/color_scheme-5.scss');
...
function theme_eguru_render_spotimg($p, $sliname) {
    global $PAGE, $OUTPUT;
    ...
    // Get slide image or fallback to default.
    //if (theme_eguru_get_setting22($sliname)) {
        $slideimage = $PAGE->theme->setting_file_url($sliname , $sliname);
    //}
    return $slideimage;
}
...
function theme_eguru_get_filearea($setting) {
    global $CFG;
    require_once($CFG->dirroot . '/lib/weblib.php');
    if (empty($theme)) {
        $theme = theme_config::load('eguru');
	...
	global $DB;
	$spotimg = $DB->get_records_sql("Select filename from modl_files where filearea = '$setting' and filesize > 0");
	//$spotimg = $DB->get_record('files',array('filearea' => $setting,'filesize' ), '*', MUST_EXIST)
	//$spotimg = 'Select filename from modl_files where filearea = ' .$setting. ' and filesize > 0';
	return $theme->settings->$setting;
}
