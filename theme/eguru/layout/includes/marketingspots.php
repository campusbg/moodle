...
function eguru_marketingspot() {
	...
	https://campusbg.bancoguayaquil.com/campusbg/message/index.php

    for ($i = 1; $i <= 4; $i++) {
    switch ($i) {
        case 4:
            $ref = 'https://campusbg.bancoguayaquil.com/campusbg/course/view.php?id=25#section-0';
            break;
        case 3:
            $ref = 'https://campusbg.bancoguayaquil.com/campusbg/course/view.php?id=7';
            break;
        case 2:
            $ref = 'https://campusbg.bancoguayaquil.com/campusbg/my/index.php?myoverviewtab=courses';
            break;
        case 1:
        ...
        if (!empty($mspicon) || !empty($msptitle) || !empty($mspdescription)) {
            $content .= html_writer::start_tag('div', array('class' => $class));
                ...
                $content .= html_writer::start_tag('div', array('class' => 'ebox', style => 'horizontal-align:middle; aling-items: center; justify-content: center; text-align: center;'));
            if (!empty($mspicon)) {
...
//style => 'background-color: rgba(18, 104, 179, 0.75);'
                $content .= html_writer::start_tag('div', array('class' => 'ebox-body'));
		    //Links
		    $content .= html_writer::start_tag('a', array('href' => $ref, 'class' => 'btn-marketing', 'style' => 'width: 60%; color: #072B4B; font-weight: 300; style="font-family: sans-serif;"'));		
